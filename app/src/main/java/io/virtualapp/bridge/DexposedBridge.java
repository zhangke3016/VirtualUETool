...
public final class DexposedBridge {
    ...
    private static HashMap<Member, XC_MethodHook> prepareHookCache = new HashMap<>();
...
//	}
    ...
    public static void addPrepareHook(Member member, XC_MethodHook XCMethodHook) {
    ...
    public static synchronized void hookManyMethod(HashMap<Member, XC_MethodHook> membersAndCallBacks) {

        ArrayList<GenedClassInfo> genedClassInfos = new ArrayList<>();
        for (Map.Entry<Member, XC_MethodHook> entry : membersAndCallBacks.entrySet()) {
    ...
    public static synchronized void hookMethod(Member hookMethod, XC_MethodHook callBack) {
