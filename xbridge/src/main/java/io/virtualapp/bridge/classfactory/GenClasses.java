...
public class GenClasses {
    ...
    private static void genField(ClassVisitor cv) {
        cv.visitField(Opcodes.ACC_PUBLIC + Opcodes.ACC_STATIC, GenedClassInfo.paramsFieldName,
                XMethodHookParams.desc, null, null).visitEnd();
        cv.visitField(Opcodes.ACC_PUBLIC + Opcodes.ACC_STATIC, GenedClassInfo.callBackFieldName, DXC_MethodHook.desc, null,
                null).visitEnd();
    }
    ...
    private static void genConstructor(ClassVisitor cv, String genClassDesc) {
        ...
        MethodVisitor mvInit = cv.visitMethod(Opcodes.ACC_PUBLIC + Opcodes.ACC_CONSTRUCTOR, FinalStr.initFuncName,
                FinalStr.voidName + FinalStr.memberDesc + DXC_MethodHook.desc, null, null);
        mvInit.visitCode();
        ...
        // BudCallBack0.param = new XMethodHookParams();
        mvInit.visitTypeInsn(Opcodes.INSN_NEW_INSTANCE, 0, 0, 0, XMethodHookParams.desc);
        mvInit.visitMethodInsn(Opcodes.INSN_INVOKE_DIRECT, XMethodHookParams.desc, FinalStr.initFuncName,
                FinalStr.voidName, new int[]{0});
        mvInit.visitFieldInsn(Opcodes.INSN_SPUT_OBJECT, genClassDesc, GenedClassInfo.paramsFieldName,
                XMethodHookParams.desc, 0, 0);
        // BudCallBack0.param.method = arg2;
        mvInit.visitFieldInsn(Opcodes.INSN_SGET_OBJECT, genClassDesc, GenedClassInfo.paramsFieldName,
                XMethodHookParams.desc, 0, 0);
        mvInit.visitFieldInsn(Opcodes.INSN_IPUT_OBJECT, XMethodHookParams.desc,
                XMethodHookParams.methodFieldName, FinalStr.memberDesc, 2, 0);
        // BudCallBack0.callBack = arg3;
        mvInit.visitFieldInsn(Opcodes.INSN_SPUT_OBJECT, genClassDesc, GenedClassInfo.callBackFieldName,
                DXC_MethodHook.desc, 3, 0);
    ...
    private static void genReplaceMethod(ClassVisitor cv, final GenedClassInfo genedClassInfo) {
        ...
        // GenClass.class 0
        // XMethodHookParams params = getMethodParams(clazz);1
        // DXC_MethodHook callBack = getCallBack(clazz);2
        ...
        mvReplace.visitMethodInsn(Opcodes.INSN_INVOKE_STATIC, GenedClassHelper.desc, GenedClassHelper.getMethodHookParamsMethodName,
                XMethodHookParams.desc + FinalStr.classDesc, new int[]{0});
        ...
        mvReplace.visitMethodInsn(Opcodes.INSN_INVOKE_STATIC, GenedClassHelper.desc, GenedClassHelper.getXCallBackMethodName,
                DXC_MethodHook.desc + FinalStr.classDesc, new int[]{0});
        ...
        mvReplace.visitFieldInsn(Opcodes.INSN_IPUT_OBJECT, XMethodHookParams.desc,
                XMethodHookParams.argsFieldName, FinalStr.bracket + FinalStr.objectDesc, 0, 1);
        ...
        if (!isStatic) {
            mvReplace.visitFieldInsn(Opcodes.INSN_IPUT_OBJECT, XMethodHookParams.desc,
                    XMethodHookParams.thisObjectFieldName, FinalStr.objectDesc, regIndex, 1);
            paramIndex++;
            regIndex++;
        }
        ...
        // call before call
        mvReplace.visitMethodInsn(Opcodes.INSN_INVOKE_VIRTUAL, DXC_MethodHook.desc, DXC_MethodHook.beforeCallMethodName,
                FinalStr.voidName + XMethodHookParams.desc, new int[]{2, 1});
        ...
        regIndex = localRegNo;
        if (!isStatic) {
            mvReplace.visitFieldInsn(Opcodes.INSN_IGET_OBJECT, XMethodHookParams.desc,
                    XMethodHookParams.thisObjectFieldName, FinalStr.objectDesc, localRegNo, 1);
            paramIndex++;
            regIndex++;
        }
        ...
        mvReplace.visitMethodInsn(Opcodes.INSN_INVOKE_VIRTUAL, XMethodHookParams.desc, XMethodHookParams.isEarlyReturnMethodName,
        ...
        if (hookedMethodInfo.hasReturn()) {
            ...
            mvReplace.visitFieldInsn(Opcodes.INSN_IPUT_OBJECT, XMethodHookParams.desc,
                    XMethodHookParams.resultFieldName, FinalStr.objectDesc, 3, 1);
        }

        mvReplace.visitLabel(lb_earlyReturn);
        // call after call
        mvReplace.visitMethodInsn(Opcodes.INSN_INVOKE_VIRTUAL, DXC_MethodHook.desc, DXC_MethodHook.afterCallMethodName,
                FinalStr.voidName + XMethodHookParams.desc, new int[]{2, 1});
        ...
        if (hookedMethodInfo.hasReturn()) {
            mvReplace.visitFieldInsn(Opcodes.INSN_IGET_OBJECT, XMethodHookParams.desc,
                    XMethodHookParams.resultFieldName, FinalStr.objectDesc, 0, 1);
        ...
        if (hookedMethodInfo.hasThrowable()) {//
            ...
            mvReplace.visitFieldInsn(Opcodes.INSN_IPUT_OBJECT, XMethodHookParams.desc,
                    XMethodHookParams.throwableFieldName, FinalStr.throwableDesc, 0, 1);
